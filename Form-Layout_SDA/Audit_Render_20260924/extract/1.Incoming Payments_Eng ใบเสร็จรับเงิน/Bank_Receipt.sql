-- ============================================================
-- Report: 1.Incoming Payments_Eng ใบเสร็จรับเงิน.rpt
-- Path:   1.Incoming Payments_Eng ใบเสร็จรับเงิน.rpt
-- Extracted: 2026-09-24 10:20:13
-- Source: Main Report
-- Table:  Bank_Receipt
-- ============================================================

-- RPT: 4. Banking\1. Incoming Payments\1.Incoming Payments_ใบเสร็จรับเงิน\1.Incoming Payments_Eng ใบเสร็จรับเงิน.rpt
-- SCOPE: main
-- ALIAS: Bank_Receipt
-- FIELDS: 49
-- ---------------------------------------------------------------
SELECT DISTINCT
    ...
    ORCT.DocCurr, 
    CASE
		WHEN ORCT.DocCurr = 'THB' THEN ORCT.DocTotal
		ELSE ORCT.DocTotalFC
	END AS 'DocTotal',
	CASE
		WHEN ORCT.DocCurr = 'THB' THEN RCT2.SumApplied
		ELSE RCT2.AppliedFC
	END AS 'LineTotal',
	RCT1.CheckNum,
    --ORCT.DocTotal,
    ...
    OCPR.E_MailL
,
    (SELECT TOP 1 PJN.PrjName FROM OPRJ PJN WHERE PJN.PrjCode = (-- ===== เพิ่ม Field Project ระดับ Line =====
    CASE 
        WHEN RCT2.InvType = '13' THEN (SELECT TOP 1 Project FROM INV1 WHERE DocEntry = RCT2.BASEABS AND ISNULL(Project, '') <> '')
        WHEN RCT2.InvType = '14' THEN (SELECT TOP 1 Project FROM RIN1 WHERE DocEntry = RCT2.BASEABS AND ISNULL(Project, '') <> '')
        WHEN RCT2.InvType = '203' THEN (SELECT TOP 1 Project FROM DPI1 WHERE DocEntry = RCT2.BASEABS AND ISNULL(Project, '') <> '')
