-- ============================================================
-- Report: Withholding Tax certificate (03) v28.rpt
-- Path:   Withholding Tax certificate (03) v28.rpt
...
---------------------------------------------------------------------START WITH FTTV ----------------------------------------------------------------------------
		...
		FROM (
				...
				AND JDT2.Account = (SELECT AcctCode FROM OACT WHERE FormatCode = (SELECT [Name] FROM  [@SLDT_SET_ACCWH] WHERE U_SLD_TypeWH = '03'))
				WHERE JDT1.Credit > 0 				
				----------------------------------------------------------------------------------------------------------------------
				UNION ALL 
				----------------------------------------------------------------------------------------------------------------------
				SELECT DISTINCT	'OJDT' AS TYPE,
						...
						JDT1.U_SLD_LPBranch AS U_SLD_VBPBranch,
						JDT1.U_SLD_WhPayBy,
						(SELECT top 1 OWHT.WTCode FROM  OWHT WHERE OWHT.WTCode = JDT1.U_SLD_WTCode AND JDT1.ShortName = (SELECT top 1 AcctCode FROM  OACT WHERE FormatCode = (SELECT top 1 [Name] FROM  [@SLDT_SET_ACCWH] WHERE U_SLD_TypeWH = '03'))) AS WTCode
						
				FROM JDT1 
				LEFT JOIN OJDT  ON JDT1.TransId = OJDT.TransId
				WHERE JDT1.ShortName = (SELECT top 1 AcctCode FROM OACT WHERE FormatCode = (SELECT [Name] FROM  [@SLDT_SET_ACCWH] WHERE U_SLD_TypeWH = '03'))                                           
			) AS B 
