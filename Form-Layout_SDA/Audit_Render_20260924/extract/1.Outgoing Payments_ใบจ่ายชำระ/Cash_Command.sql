...
+ case when isnull(T1.[Segment_9],'') = '' then '' Else '-' + T1.[Segment_9] end as FormatCode, T1.[AcctName],  T0.[CashSum], T0.[CashSumFC] FROM OVPM T0  INNER JOIN OACT T1 ON T0.[CashAcct] = T1.AcctCode WHERE T0.[DocNum] = '{?Docnum}'
