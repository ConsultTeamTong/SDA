...
-- RPT: 2. Sales - AR\7. AR Credit Memo\4.AR_Credit Memo_ใบลดหนี้_(Batch_Serial) (Dis)\1.AR_Credit Memo_ใบลดหนี้_ENG (Batch_Serial) (Dis).rpt
-- SCOPE: main
-- ALIAS: AR_CN
-- FIELDS: 56
-- ---------------------------------------------------------------
...
ISNULL(NULLIF(OUOM.U_SLD_Uomforeign,''), RIN1.UomCode) AS UgpCode,
...
LEFT JOIN OUOM ON RIN1.UomCode = OUOM.UomCode
LEFT JOIN [dbo].[@SLDT_SET_BRANCH] BRANCH ON ORIN.U_SLD_LVatBranch = BRANCH.Code 
LEFT JOIN OINV Ref_OINV ON RIN1.BaseEntry = Ref_OINV.DocEntry 
                        AND RIN1.BaseType  = 13  -- 13 = A/R Invoice
