-- ============================================================
-- Report: 2.AP Down Payment Invoice_ENG ใบจ่ายเงินมัดจำใบกำกับภาษี.rpt
-- Path:   2.AP Down Payment Invoice_ENG ใบจ่ายเงินมัดจำใบกำกับภาษี.rpt
-- Extracted: 2026-09-24 10:20:33
-- Source: Main Report
-- Table:  AP_DownPayment
-- ============================================================

-- RPT: 3. Purchasing - AP\6. AP Down Payment Invoice\1.AP Down Payment ใบจ่ายเงินมัดจำ\1.AP Down Payment_ENG ใบจ่ายเงินมัดจำ.rpt
-- SCOPE: main
...
CASE
	WHEN ODPO.DocCur = 'THB' THEN DPO1.LineTotal
	ELSE DPO1.TotalFrgn
END AS 'LineTotal',

CASE
	WHEN ODPO.DocCur = 'THB' THEN ODPO.DiscSum
	ELSE ODPO.DiscSumFC
END AS 'DiscSum',

CASE
	WHEN ODPO.DocCur = 'THB' THEN ODPO.VatSum
	ELSE ODPO.VatSumFC
END AS 'VatSum',

CASE
	WHEN ODPO.DocCur = 'THB' THEN ODPO.DocTotal
	ELSE ODPO.DocTotalFC
END AS 'DocTotal',
    (SELECT CASE WHEN ODPO.DocCur = 'THB' THEN SUM(L.LineTotal) ELSE SUM(L.TotalFrgn) END
        FROM DPO1 L WHERE L.DocEntry = ODPO.DocEntry) AS 'Sum_LineTotal_All',
...
ISNULL(NULLIF(OUOM.U_SLD_Uomforeign,''), DPO1.UomCode) AS UgpCode
,
...
LEFT JOIN OITM ON DPO1.ItemCode = OITM.ItemCode
LEFT JOIN OUOM ON DPO1.UomCode = OUOM.UomCode
...
CRD1.Country,
OITM.FrgnName,
ISNULL(NULLIF(OUOM.U_SLD_Uomforeign,''), DPO1.UomCode),
ODPO.DiscSum,
ODPO.DiscSumFC,
ODPO.VatSumFC,
ODPO.DocTotal
...
END AS 'DocTotal',
    (SELECT CASE WHEN ODPO.DocCur = 'THB' THEN SUM(L.LineTotal) ELSE SUM(L.TotalFrgn) END
        FROM DRF1 L WHERE L.DocEntry = ODPO.DocEntry) AS 'Sum_LineTotal_All',
